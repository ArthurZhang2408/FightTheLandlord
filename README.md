...
5. First bidder: every new match (including “沿用上次玩家，再开一场”) starts with seat A; each game then passes to the next seat. A round nobody bids on is not recorded and rotates the first bidder once more; the seat menu on the board sets it explicitly, and the 换位 button on the scoreboard card moves everyone one seat to the left (A, B, C → B, C, A) with every per-seat value following its player. Before the first round the first bidder is a seat, so whoever is moved into seat A bids first; once rounds exist the turn belongs to a person and moves with them. `MatchSession` reads `nextFirstBidder` into a local before writing `active` (an optional-chain write whose right-hand side reads the same `@Observable` property is an exclusivity trap at runtime).
